...
(function(global){
  ...
  async function sheetInsert(rows){
    ...
    try{
      const res = await fetch(SHEETDB_API_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ data: rows }) });
      return res.ok;
    }catch{ return false; }
  }
  async function recordTransferToSheet(t){
    try { if(!t || !t.reference) return; const existing = await sheetSearch({ formType:'Transfer', reference: t.reference }); if(Array.isArray(existing) && existing.length) return; const row = { formType: 'Transfer', reference: t.reference, email: t.email || (getUser() && getUser().email) || '', amount: t.amount, from: t.from || t.from_account || '', to: t.to || t.to_account || '', status: t.status || 'Pending', dateISO: t.dateISO || new Date().toISOString(), network: t.type || t.network || 'Transfer' }; await sheetInsert([row]); } catch {} }
  ...
  global.Platform = Object.assign(global.Platform||{}, { getUser, getApiToken, recordTransferToSheet, fetchTransfersUnified, fetchTransferByRef });
})(window);
